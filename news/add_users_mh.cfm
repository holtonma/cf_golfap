...
<table cellpadding="0" cellspacing="0" class="tabSpaceMed">
	...
	<tr>
		...
		<td><input type="text" id="cdl_output" value="" size="80" disabled></td>
	</tr>
	<tr>
...
<script type="text/javascript">
  function process(){
	...
	$$('.W100').each(function(f){
	  //if f.value.length = 6, that's a CUID... mu
  ...
  //this is attaching the above function to the button:
  Event.observe(window, 'load', function(){
    console.log("window loaded...");
    Event.observe('btnProcess', 'click', process.bind($('btnProcess')));
  });
</script>
